...
// Single consolidated message listener at module scope
	...
	if (allow_selection()) {
		...
		if (timer !== 0) {
			...
			if (box === null) {
				...
				count_label.style.visibility = "hidden";
				// Reads as extension chrome rather than page content, so it stays
				// legible whatever the page underneath looks like.
...
function detech(x, y, open) {
	...
	// Clear open_tabs at the start of each "open" pass
	if (open) {
		open_tabs = [];
	}
